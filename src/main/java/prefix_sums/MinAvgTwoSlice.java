...
public class MinAvgTwoSlice {
    ...
    public int solution(int[] A) {
        ...
        for (int i = 0; i < A.length - 1; i++) {
            ...
            //idea is that global minimum average value will be within slices of size 2 or 3
            double twoSlice = (A[i] + A[i+1]) / 2.0;
            double threeSlice = Double.MAX_VALUE;

            if(i < A.length - 2) {
                threeSlice = (A[i] + A[i+1] + A[i+2]) / 3.0;
            }
            double curMinAvg = Math.min(threeSlice, twoSlice);
            if(curMinAvg < minAvgValue) {
                minAvgValue = curMinAvg;
                resultIndx = i;
            }
        }
        return resultIndx;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.solution(new int[] {4, 2, 2, 5, 1, 5, 8})); //should return 1
        System.out.println(solution.solution(new int[] {-3, -5, -8, -4, -10})); //should return 2
    }
}
