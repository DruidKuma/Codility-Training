...
public class FrogImp {
    ...
    public int solution(int X, int Y, int D) {
        int distance = Y - X;
        return distance / D + (distance % D == 0 ? 0 : 1); // add 1 extra jump if not enough
    }

    public static void main(String[] args) {
        FrogImp solution = new FrogImp();

        System.out.println(solution.solution(10, 85, 30)); // should return 3
    }
}
